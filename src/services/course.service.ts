...
export const getCourses = async (
  ...
  try {
    ...
    // Apply filters
    if (subject || format || seminarDayId || search) {
      options.where = {};

      if (subject) {
        options.where = { ...options.where, category: subject };
      }

      if (format) {
        options.where = { ...options.where, format };
      }

      if (seminarDayId) {
        options.where = { ...options.where, seminarDayId };
      }

      // Handle search separately since it needs LIKE operator
      if (search) {
        // We'll handle search in a separate query
        const searchIds = await courseRepository
        ...
        if (searchIds.length > 0) {
          options.where = { ...options.where, id: In(searchIds.map(c => c.id)) };
        } else {
    ...
    const [courses, total] = await courseRepository.findAndCount(options);
